...
<div>
	<div class="row">
		<div class="col-md-offset-1 col-md-10" align="left">
			<h3>
				'<span
					style="background-color: #ff9400; color: white; padding: 3px;">${keyword }</span>'을(를)
				${type }(으)로 검색 결과
			</h3>
		</div>
				...
				<div class="col-md-1">
					<c:if test="${result.size() gt 100 }">
						<c:if test="${page le (max%10 eq 0 ? max-10 : max-max%10 )}">
							<a
								href="/search/list?keyword=${keyword }&type=${type eq '상호' ? 'title' : type eq '위치' ? 'location' : ''}&page=${page%10 eq 0 ? page+1 : page-(page%10)+11 }" style="font-size: 20px;">NEXT</a>
						</c:if>
					</c:if>
				</div>
			</div>
